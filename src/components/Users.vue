			...
			<div class="col-sm-12 col-md-4" v-for="user in page" :key="user.id">
				<div class="flip-card">
					<div class="flip-card-inner">
						...
						<div class="">
							<h5>{{ user.name }}</h5>
							<p><span class="text-secondary fw-bolder">Age: </span>{{ user.age }}</p>
							<p><span class="text-secondary fw-bolder">Gender: </span>{{ user.gender }}</p>
							<p><span class="text-secondary fw-bolder">Eye Color: </span>{{ user.eyeColor }}</p>
						</div>
						</div>
						<div class="flip-card-back bg-info">
							<p><span>Id: </span>{{ user._id }}</p>
							<p><span>Latitude: </span>{{ user.location.latitude }}</p>
							<p><span>Longitude: </span>{{ user.location.longitude }}</p>
							<p><span>Pet: </span>{{ user.preferences.pet }}</p>
							<p><span>Fruit: </span>{{ user.preferences.fruit }}</p> 
						</div>
					</div>
				</div>
			</div>
	...
	setup() {
		...
		const people = computed(() =>
			allPeople.value.filter((person) => {
				...
				const byName = person.name.toLowerCase().includes(search.value.toLowerCase());
